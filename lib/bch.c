 ...
 *
 * Call init_soft_bch to get a pointer to a newly allocated bch_control structure for
 * the given m (Galois field order), t (error correction capability) and
 * (optional) primitive polynomial parameters.
 *
 * Call encode_soft_bch to compute and store ecc parity bytes to a given buffer.
 * Call decode_soft_bch to detect and locate errors in received data.
 *
 * On systems supporting hw BCH features, intermediate results may be provided
 * to decode_soft_bch in order to skip certain steps. See decode_soft_bch() documentation
 * for details.
 *
...
/*
 * same as encode_soft_bch(), but process input data one byte at a time
 */
static void encode_soft_bch_unaligned(struct bch_control *bch,
...
/**
 * encode_soft_bch - calculate BCH ecc parity of data
 ...
 */
void encode_soft_bch(struct bch_control *bch, const uint8_t *data,
		unsigned int len, uint8_t *ecc)
{
	...
	if (m) {
		...
		encode_soft_bch_unaligned(bch, data, mlen, bch->ecc_buf);
		data += mlen;
		len  -= mlen;
	}
	...
	if (len)
		encode_soft_bch_unaligned(bch, data, len, bch->ecc_buf);

	/* store ecc parity bytes into original parity buffer */
	if (ecc)
		store_ecc8(bch, ecc, bch->ecc_buf);
}
// EXPORT_SYMBOL_GPL(encode_soft_bch);
...
/**
 * decode_soft_bch - decode received codeword and find bit error locations
 * @bch:      BCH control structure
 ...
 * separately (using encode_soft_bch()), this function should be called with one of
 * the following parameter configurations -
 *
 * by providing @data and @recv_ecc only:
 *   decode_soft_bch(@bch, @data, @len, @recv_ecc, NULL, NULL, @errloc)
 *
 * by providing @recv_ecc and @calc_ecc:
 *   decode_soft_bch(@bch, NULL, @len, @recv_ecc, @calc_ecc, NULL, @errloc)
 *
 * by providing ecc = recv_ecc XOR calc_ecc:
 *   decode_soft_bch(@bch, NULL, @len, NULL, ecc, NULL, @errloc)
 *
 * by providing syndrome results @syn:
 *   decode_soft_bch(@bch, NULL, @len, NULL, NULL, @syn, @errloc)
 *
 * Once decode_soft_bch() has successfully returned with a positive value, error
 ...
 */
int decode_soft_bch(struct bch_control *bch, const uint8_t *data, unsigned int len,
	       ...
	       const unsigned int *syn, unsigned int *errloc)
{
	...
	if (!syn) {
		if (!calc_ecc) {
			...
			if (!data || !recv_ecc)
				return -EINVAL;
			encode_soft_bch(bch, data, len, NULL);
		} else {
...
// EXPORT_SYMBOL_GPL(decode_soft_bch);
...
/**
 * init_soft_bch - initialize a BCH encoder/decoder
 ...
 * path. Usually, init_soft_bch() should be called on module/driver init and
 * free_soft_bch() should be called to release memory on exit.
 *
 * You may provide your own primitive polynomial of degree @m in argument
 * @prim_poly, or let init_soft_bch() use its default polynomial.
 *
 * Once init_soft_bch() has successfully returned a pointer to a newly allocated
 * BCH control structure, ecc length in bytes is given by member @ecc_bytes of
 * the structure.
 */
struct bch_control *init_soft_bch(int m, int t, unsigned int prim_poly)
{
	int err = 0;
	...
	if ((m != (CONFIG_BCH_CONST_M)) || (t != (CONFIG_BCH_CONST_T))) {
		printk(KERN_ERR "bch encoder/decoder was configured to support "
		       "parameters m=%d, t=%d only - values m=%d t=%d!\n",
			CONFIG_BCH_CONST_M, CONFIG_BCH_CONST_T, m, t);
		goto fail;
	}
#endif
	if ((m < min_m) || (m > max_m))
...
// EXPORT_SYMBOL_GPL(init_soft_bch);

/**
 *  free_soft_bch - free the BCH control structure
 *  @bch:    BCH control structure to release
 */
void free_soft_bch(struct bch_control *bch)
{
	unsigned int i;
...
// EXPORT_SYMBOL_GPL(free_soft_bch);
