...
ALL = MLO x-load.bin.ift System.map
...
x-load:	$(VERSION_FILE) $(TIMESTAMP_FILE) $(OBJS) $(LIBS) $(LDSCRIPT)
...
x-load.bin.ift:	x-load.bin scripts/signGP
	scripts/signGP x-load.bin $(TEXT_BASE)

$(TIMESTAMP_FILE):
		@date +'#define X_LOADER_DATE "%F"' > $@
		@date +'#define X_LOADER_TIME "%T"' >> $@

$(VERSION_FILE):
		( printf '#define X_LOADER_VERSION "X-Loader %s%s %s"\n' "$(X_LOADER_VERSION)" \
...
all install x-load x-load.srec oneboot depend dep $(VERSION_FILE):
...
dm3730logic_config :       unconfig
	@./mkconfig $(@:_config=) arm omap3 dm3730logic
